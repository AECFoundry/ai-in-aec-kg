...
STAGES = ["parse", "extract", "resolve", "enrich", "embed", "load"]
...
def run_embed(
    ...
    embeddings = _run_embed(extraction.entities, sessions_data, api_key)
...
def main():
    ...
    for stage in stages_to_run:
        ...
        if stage == "parse":
            ...
            enrich_data = run_enrich(resolve_data, sessions_data)

        elif stage == "embed":
            embeddings = run_embed(enrich_data, sessions_data)

        elif stage == "load":
            run_load(enrich_data, sessions_data, embeddings)
